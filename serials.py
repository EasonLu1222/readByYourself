...
from serial import SerialException                              #引進serial
...
PADDING = ' ' * 4                                             #定義PADDING為空字串



def get_serial(port_name, baudrate, timeout):                  #定義serial函式設定
    #  logger.info(f'{PADDING}===get_serial=== {port_name}')   #debug用
...
def is_serial_free(port_name):                                 #設定port_name
    try:
